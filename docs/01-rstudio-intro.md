...
Open command prompt (Open Start Menu then type `cmd` and press [Enter]). Make sure your command prompt opens at the C directory e.g. ```C:\>```. If it opens in another directory you can change this by typing ```C:```.  
...
![RStudio layout with .R file open](fig/rmd-01-rstudio-script.png)

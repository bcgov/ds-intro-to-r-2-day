...
## Example of Using rmarkdown in the BC Government

R Markdown is quite versatile  
...
If we specified our output to be a html file, the Markdown file (and figure documents) is then converted to a html file using a tool called [`pandoc`](http://pandoc.org/). Different output file types require different tool for conversion. This will be dicussed below. 
...
Different output file types require different tool for conversion. This will be dicussed below. 

## Chunk options
 
As we saw above, code chunks are a very important part of using Rmarkdown. We will now explore some of the options for controlling how these will show up in our document. 
...
We can use the **gapminder** dataset and `ggplot2`package to create and format a plot within RMarkdown. 

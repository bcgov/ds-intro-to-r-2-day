...
> ## Challenge 1
>
> Up until R version 4.0, the default behaviour of `read.csv` is to convert all
> character columns to factors, which has been one of the biggest reasons we
> prefer `read_csv` from the `readr` package. As of version 4.0, this behaviour
> has changed and now the default is to leave character columns as character.
> Look at the help for `?read.csv` to figure out how to control whether character 
> columns are read as as character vectors or factors; then write a command or 
> two to check that the column in `cats` is what you expect it to be.
>

...
keypoints:
- "Accessing data directly from the B.C. Data Catalogue using the bcdata R package." 
source: Rmd
---
...
# Introduction to bcdata package
...
Looks useful, we can use the `bcdc_get_data` function to load this data directly into R:

```{r}
libs_raw <- bcdc_get_data(record = 'bc-public-libraries-systems-branches-and-locations', resource = 'ed17f111-fb39-46b3-89aa-5375592fbb01')
```
...
Let's return to the libraries data. As we learned yesterday morning, a useful first step is to get familiar with the data.
...
Let's tidy up this data frame and find out how many physical materials or volumes are available within each City:
...
Now that we have imported, tidied, and analyzed our library data, let's make a static data visualization to share the results with others.
...
>
> Iterate the above plot to also visualize the number of books by Library >Service. Add a title and try other design changes to make the plot more >readable.
>
>
>
>```{r}
